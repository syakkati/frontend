...
class MyBudgetsComponent extends Component {
  ...
  async componentDidMount() {
    ...
    const response = await axios.get("http://3.218.18.30:3001/app/userBudget", {
      headers: {
    ...
    if (editedItemName.length == 0 || editedBudgetValue == null) {
      ...
      try {
        ...
        const response = await axios.put(
          "http://3.218.18.30:3001/app/userBudget",
          {
  ...
  handleDeleteConfirmation = async () => {
    ...
    try {
      await axios.delete('http://3.218.18.30:3001/app/userBudget/', { budget_id: budgetToDelete.budget_id }, {
        headers: {
          Authorization: 'Bearer ' + token,
          'Content-Type': 'application/json',
        },
