...
function createData(sno, configure_id, user_id, category, item, budget) {
    return {
        sno,
        configure_id,
        user_id,
...
class MyConfigureBudgetsComponent extends Component {
    constructor(props) {
        super(props);
        this.state = {
            rows: [],
    ...
    async componentDidMount() {
        ...
        const response = await axios.get("http://3.218.18.30:3001/app/userConfigureBudgets", {
            headers: {
    ...
    handleUpdateBudget = async () => {
        const token = localStorage.getItem("token");
        const { editedRow, editedCategory, editedItemName, editedBudgetValue } = this.state;

        if (editedCategory.length == 0 || editedItemName.length == 0 || editedBudgetValue == null) {
            ...
            try {
                ...
                const response = await axios.put(
                    "http://3.218.18.30:3001/app/userConfigureBudgets",
                    {
                        configure_id: editedRow.configure_id,
    ...
    handleNewCategoryValueChange = (event) => {
        this.setState({ newCategoryName: event.target.value });
    };

    handleAddNewConfigureBudget = async () => {
        const token = localStorage.getItem("token");
        if (this.state.newCategoryName.length == 0 || this.state.newItemName.length == 0 || this.state.newBudgetValue == null) {
            alert("Please enter all required fields");
        }
        else {
    ...
    handleDeleteConfirmation = async () => {
        ...
        try {
            await axios.delete('http://3.218.18.30:3001/app/userBudget/',
                { configure_id: budgetToDelete.configure_id },
                {
                    headers: {
                        Authorization: 'Bearer ' + token,
                        'Content-Type': 'application/json',
            ...
            const updatedRows = this.state.rows.filter(
                (row) => row.configure_id !== budgetToDelete.configure_id
            );
    ...
    render() {
        const { isUpdateDialogOpen, editedCategory, editedItemName, editedBudgetValue } = this.state;
                        ...
                        >
                            ...
                            <DialogContent>
                                <TextField
                                    ...
                                    onChange={this.handleNewCategoryValueChange}
                                    fullWidth
                                    sx={{ mb: 2 }}
                                    helperText={'Category Name is required'} // Display error message
                            ...
                            <DialogActions>
                                ...
                                <Button onClick={this.handleAddNewConfigureBudget}>Add</Button>
                            </DialogActions>
                        </Dialog>
                        ...
                        >
                            ...
                            <DialogContent>
                                <TextField
                                    ...
                                    onChange={(e) => this.setState({ editedCategory: e.target.value })}
                                    fullWidth
                                    sx={{ mb: 3 }}
...
export default MyConfigureBudgetsComponent;