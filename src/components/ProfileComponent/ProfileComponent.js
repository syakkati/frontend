...
const ProfileComponent = () => {
    ...
    const fetchUserData = async () => {
        try {
            const response = await axios.get('http://3.218.18.30:3001/app/userProfile', {
                headers: {
    ...
    const handleUpdateClick = async () => {
        ...
        try {
            // console.log(editedUserData);
            const response = await axios.put('http://3.218.18.30:3001/app/userDetails', editedUserData, {
                headers: {
                    Authorization: `Bearer ${localStorage.getItem('token')}`,
                },
            });
